...
if (isset($_POST['PURCHASE'])) {
    if (empty($_POST['address'])) {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            foreach ($p_id as $id) {
                if ($row['id'] == $id) {
                    $totalPrice += (int)$row['p_price'];
                }
            }
        }

        $sql = "INSERT INTO orders (userid, address, price) VALUES (?, ?, ?)";
        $q = mysqli_stmt_init($con);
        if (mysqli_stmt_prepare($q, $sql)) {
            mysqli_stmt_bind_param($q, 'iss', $user['id'], $_POST['address'], $totalPrice);
        ...
        if (mysqli_stmt_prepare($q1, $sql)) {
            mysqli_stmt_bind_param($q1, 'i', $user['id']);
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            foreach ($p_id as $id) {
                if ($row['id'] == $id) {
                    $sql = "INSERT INTO orderdetails (orderid, userid, productid, amount, price) VALUES (?, ?, ?, 1, ?)";
                    $q = mysqli_stmt_init($con);
                    if (mysqli_stmt_prepare($q, $sql)) {
                            ...
                            <div class="col-lg-4 bg-grey">
                                <div class="p-5">
                                    ...
                                    <form action="" method="POST">
                                        ...
                                        <button type="submit" name="PURCHASE" class="btn btn-dark btn-block btn-lg" data-mdb-ripple-color="dark">PURCHASE</button>
                                    </form>
                                </div>
                            </div>
                        </div>
